...
from keras.layers import Embedding, GlobalAveragePooling1D, Dense, Reshape, concatenate, BatchNormalization, LSTM
...
class HeadlineTimeLstmBodyBeginModelBuilder(ModelBuilder):
    MODEL_IDENTIFIER = 'headline_time_lstm_body_begin_model'
    ...
    def __call__(self):
        ...
        body_embedding = Embedding(glove.embedding_vectors.shape[0],
                                   glove.embedding_vectors.shape[1],
                                   weights=[glove.embedding_vectors],
                                   trainable=False)(body_begin_input)

        lstm = LSTM(self.default_parameters['lstm_units'])(body_embedding)
        lstm_out = Dense(1, activation='sigmoid')(lstm)
        ...
        model = Model(inputs=[headline_input,
                              ...
                              day_of_week_input,
                              day_of_year_input,
                              body_begin_input], outputs=[headline_output, main_output], name=self.MODEL_IDENTIFIER)
...
class HeadlineTimeLstmBodyBeginPreprocessor(Preprocessor):

    def __init__(self, model, glove, max_headline_length, body_begin_length):
...
def train():
    ...
    model_builder = HeadlineTimeLstmBodyBeginModelBuilder() \
    ...
    preprocessor = HeadlineTimeLstmBodyBeginPreprocessor(model, glove, arguments.max_headline_length,
                                                         arguments.body_begin_length)
    ...
    training_input = [preprocessor.training_data['headlines'],
                      ...
                      preprocessor.training_data['body_beginnings']]
    ...
    validation_input = [preprocessor.validation_data['headlines'],
                        ...
                        preprocessor.validation_data['body_beginnings']]
