...
class HeadlineTfIdfModelBuilder(ModelBuilder):
    ...
    MODEL_IDENTIFIER = 'headline_tfidf_model'

    def __init__(self):
        super().__init__()

        self.required_inputs.append('glove_tfidf')
    ...
    def __call__(self):
        ...
        glove_tfidf = self.inputs['glove_tfidf']
        headline_input = Input(shape=(self.parameters['max_headline_length'],), name='headline_input')
        headline_embedding = Embedding(glove_tfidf.max_words + 1,
                                       glove_tfidf.glove.DIMENSIONS,
